...
object MapScene {
  ...
  private class MapSceneImpl (override val parentStage: Stage, val _controller : MapController, var gameC :GameController, translationX : Double = 0, translationY: Double = 0) extends MapScene {
    ...
    private val _mapWindow: BorderPane = new BorderPane {
      top = menu
      center = field
    ...
    private def setPaneChildren(list :List[RectangleCell]): Unit = {
      field.children = list
    ...
    private def playerImg_(player: PlayerRepresentation):Unit = {playerImg.fill_=(new ImagePattern(new Image(player.url)))}

    private def showStatueAlert(money: Int): Unit = {
      Platform.runLater(() -> {
    ...
    private def showChestAlert(money: Int): Unit = {
      Platform.runLater(() -> {
    ...
    private def createBottomCard(): List[Button] = (0 to 4).toList.map(_ =>{
      new Button {
        var re: Cell = _
    ...
    private def icon(rectangle: RectangleCell, url: String, elemWidth: Double = 60, elemHeight: Double = 80 ): Rectangle = new Rectangle() {
    ...
    private def changeScene(user:User, enemy:Enemy): Unit = gameC.setScene(this, BattleScene(parentStage, enemy, gameC))
    ...
    override def update[A](model: A): Unit = model match {
      case player:User =>
        ...
        _controller.dashboard.remove[Enemy]()
      ...
      case (cellList: List[RectangleCell], remainingEnem : Int, changePlaceableCard: Boolean) => {
        setPaneChildren(cellList)
        remainingEnemies.set("Enemies: " + remainingEnem)
        if(changePlaceableCard) placeableCards.children = createBottomCard()
      }
      case _: Int => gameC.setScene(this,GameOverScene(parentStage, gameC))
    }
