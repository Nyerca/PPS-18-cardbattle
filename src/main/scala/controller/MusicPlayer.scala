...
object MusicPlayer {
  ...
  private def setMedia(soundType: SoundType): Option[MediaPlayer] = soundType match {
    case MapSound => Some(new MediaPlayer(new Media(getClass.getClassLoader.getResource("music/Dungeon1.wav").toString)))
    case BattleSound => Some(new MediaPlayer(new Media(getClass.getClassLoader.getResource("music/Battle1.wav").toString)))
    case WinningSound => Some(new MediaPlayer(new Media(getClass.getClassLoader.getResource("music/Winning.wav").toString)))
    case LoseSound => Some(new MediaPlayer(new Media(getClass.getClassLoader.getResource("music/Losing.wav").toString)))
  }
